...
efs <- (36-33.7)/8.7
# effect size is alternate hypothesis - null hypothesis/ standard deviation)

## Calculate the power
pwr.norm.test(d=efs, n=65, sig.level=0.05, alternative="two.sided")
# we really want .8 power (80%)
...
#                                          
# More power when a both samples are the same size
#  Example: It is known that weights of U.S. girls ages 1-6 is normally #
#  distributed with a mean of 33.1 pounds and standard deviation of     #
...
# the $power will only output power (this can be done with other vals as well)
...
power <- function(x){
  ...
  smean <- mean(samps)
  efs <-(smean - mu)/ sd
  pwr.norm.test(d=efs, n=x, sig.level=0.05, alternative="two.sided")$power
}
emPpower <- sum(replicate(1000, power(40)))/1000
emPpower

pwr.t.test(d=0.5, n=9, sig.level=0.05, type="one.sample", alternative="two.sided")

tpower <- function(x){
  samps <- rnorm(x, mu, sd)
  smean <- mean(samps)
  #efs <-(smean - mu)/ sd
  pwr.t.test(d=0.2, n=x, sig.level=0.05, alternative="two.sided")$power
}
temPpower <- sum(replicate(1000, power(100)))/1000
temPpower

# Remember this for the homework- this is how you calculate power, not simple functions

ttester1 <- function(x){
  real = 2.31
  #dfs <-39
  #mean<-dfs
  sample <- rnorm(x, 2.31, .16)
  
  tstat <- (mean(sample) - 2.30)/(sd(sample)/sqrt(x))
  pvalue <- pt(-abs(tstat), df = x-1) *2
  reject_null <- pvalue < 0.05
  reject_null
}
emppower1 <- sum(replicate(K, ttester1(40)))/10000
emppower1
emppower2 <- sum(replicate(K, ttester1(100)))/10000
emppower2
emppower3 <- sum(replicate(K, ttester1(400)))/10000
emppower3
emppower4 <- sum(replicate(K, ttester1(1000)))/10000
emppower4
emppower5 <- sum(replicate(K, ttester1(5000)))/10000
emppower5
...
rztprop<- function(x){
  samps <- rbinom(1, x, prob = 0.1)
  p_val <- prop.test(samps, x, p=0.1, alternative = "greater",correct=FALSE)$p.value
  reject_null <- (p_val < 0.05)
  reject_null
}
set.seed(3080)
t1error1b <- sum(replicate(K, rztprop(9)))/K
t1error1b
set.seed(3080)
t1error1b <- sum(replicate(K, rztprop(27)))/K
t1error1b
set.seed(3080)
t1error1b <- sum(replicate(K, rztprop(51)))/K
t1error1b
set.seed(3080)
t1error1b <- sum(replicate(K, rztprop(78)))/K
t1error1b
set.seed(3080)
t1error1b <- sum(replicate(K, rztprop(103)))/K
t1error1b

## Continuation of normal script
...
library(pwr)
###############################################
# 5.5 Daily Assignment
set.seed(1160)
samp<- runif(24)
samp2<-runif(24)
samp
samp2
Ysamp <- length(samp[samp <= .1])
Nsamp <- length(samp[(samp > .1 && samp <= .7)])
Psamp <- length(samp[samp > .7])
Ysamp2 <- length(samp2[samp2 <= .1])
Nsamp2 <- length(samp2[(samp2 > .1 && samp2 <= .7)])
Psamp2 <- length(samp2[samp2 > .7])

table1 <- matrix(c(Ysamp, Nsamp, Psamp, Ysamp2, Nsamp2, Psamp2), ncol= 2)
table1

change_vals <- function(val){
  if(val <= 0.1){
    return('Y')
...
test<- function(){
  samp<- runif(49)
  samp2<-runif(49)
  samp <- sapply(samp, change_vals)
  samp2 <- sapply(samp2, change_vals)
  x = table(factor(samp, levels = pop_vals))
  y = table(factor(samp2, levels = pop_vals))
  M = matrix(c(x, y), nrow = 2, byrow = T)
  dimnames(M) <- list(Samp<- c("first", "second"), Values =c("Y", "N", "P"))
  #test = chisq.test(M, correct = F)$p.value
  test = fisher.test(M)$p.value
  #reject_true <- test < 0.05
  #reject_true
}
q1 <- sum(replicate(10000, test()), na.rm = T)/10000
q1
