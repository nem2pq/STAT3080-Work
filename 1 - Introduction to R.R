...
Log(2.7)  #this doesn't exsist with the current packages loaded
exp(1)    # r is case sensistive
sqrt(9)
factorial(3)
abs(-5)
sum(2,4,6)
...
mult_data <- data.frame(char_vect,x)
mult_data
...
tapply(Vocab$education, Vocab$year, mean)
...
avg <- data.frame(avg_edu, avg_score)
avg
avg_cor <- tapply(x =Vocab$vocabulary,y= Vocab$education, cor)
Vyrs<-unique(Vocab$year)
Vyrs

vcor<-function(x){
  edu<-Vocab$education[which(Vocab$year==x)]
  score<-Vocab$vocabulary[which(Vocab$year==x)]
  cor(edu,score)
}

newvect<-sapply(Vyrs,vcor)
avg <- data.frame(avg_edu, avg_score, newvect)
avg
...
head(samp)
Vsummary1<-function(x){
  samp<-sample(nrow(Vocab),48)
  Vsub<-Vocab[samp,] 
...
Vsummary2<-function(x){
  Vocab.yr<-Vocab[which(Vocab[,"year"] == x),]
  samp<-sample(nrow(Vocab.yr),48)
  Vsub<-Vocab.yr[samp,]
  avgE<-mean(Vsub$education)
  avgS<-mean(Vsub$vocabulary)
  corES<-cor(Vsub$education,Vsub$vocabulary)
...
Vsummary2(1976)
lapply(unique(Vocab$year), Vsummary2)
#################
#  Replication  #
#################
