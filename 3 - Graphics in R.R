...
# color is outside/ border, fill is inside
##################### end of section 1 ##################################
...
ggplot(Davis, aes(x=weight, y=repwt, color=repht))+ geom_point(shape=0) + scale_colour_gradient(low="Gray",high = "Blue")
...
mpgPlot + facet_grid(cyl~drv, scales="free", space = "free_y")
...
ggplot(USPop, aes(x=year, y=population))+geom_line(arrow =arrow(angle= 30))
...
ggplot(uspopage, aes(x=Year, y=Thousands, color=AgeGroup)) + geom_line() + geom_point(aes(x=Year, y=Thousands), shape = 1, inherit.aes = F)
...
ggplot(bwdata, aes(x=race, y=bwt)) + geom_boxplot()+ geom_point(aes(colour=race), shape = 4)
...
library(MASS)
head(birthwt)
mid<-ggplot(birthwt, aes(x=smoke))
mid<-geom_bar()

midwest$inmetro<-ifelse(midwest$inmetro, "Y", "N")
mid<-ggplot(midwest,aes(x=state,fill=inmetro))
mid+geom_bar(position="dodge")
...
dist2 + stat_function(fun=dchisq, args=list(df=4), geom="point")
dist2 + stat_function(fun=dXXX)
...
faith = faithful[faithful$waiting > 65,]
ggplot(faith, aes(sample = waiting)) + stat_qq() + stat_qq_line()f
?stat_qq
######################################
...
DavisScatter2 + labs(title="Weights of individuals who regularly exercise",
                     x="Measured weight", y="Reported weight") +
  labs(color="Biological sex") + 
  scale_color_discrete(limits=c("M","F"), labels=c("Female", "Male"))
...
ggplot(Davis, aes(x=weight, y=repwt)) + geom_point() + theme_bw() + theme(plot.title=element_text(hjust= .5))
