...
filter(mult_data, Fives>25)  # can just use the column name, as it uses the dataframe name as a reference
filter(mult_data, Fives>25, Fours>12) #chooses the factor that includes more
filter(mult_data, Fives>25 | Fours>12)  # or
filter(mult_data, Fives>25 | Fives<10) # can uses on the same colmn to get an array of rows
...
select(mult_data, Fives:Threes) #take all columns from 5 to 3
...
summarize(mult_data, s.Fives=sum(Fives), s.Four=sum(Fours), 
          s.Threes=sum(Threes), s.Twos=sum(Twos)) #still a dataframe, not a tibble

#  The group_by() function is often used with the summarize() function  #
#  to obtain summaries for differing groups.                            #

mult_data2 <- group_by(mult_data, E_O) # much more data in the environment
...
group_by(Davis, sex) %>%
  summarize(avg.height=mean(height), avg.weight=mean(weight)) # so first input
# is the group by data set in summarize
